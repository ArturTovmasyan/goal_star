  ...
  var FOS_COMMENT = {
    ...
    post: function(url, data, success, error, complete) {
      ...
      var transformResponse = function(response){
        response = FOS_COMMENT.utcConvert(response);
        success(response);
      };
      $.post(url, data, transformResponse).error(wrappedErrorCallback).complete(wrappedCompleteCallback);
    },
    ...
    /**
     * Shorcut dateTime in timezon method.
     *
     * @param html content
     */
    utcConvert: function(html) {
      html = $(html);
      var times = html.find('.my-time');

      times.each(function(index, time){
        var t = $(time).text();
        //todo server timeZone is Europe/Berlin
        t += " +02:00";
        t = moment(t).format('MMMM DD,YYYY H:mm');
        $(time).text(t);
      });
      return html;
    },
    ...
    getThreadComments: function(identifier, permalink) {
      ...
      FOS_COMMENT.get(
        ...
        // success
        function(data) {
          data = FOS_COMMENT.utcConvert(data);
    ...
    editComment: function(commentHtml) {
      ...
      var originalCommentBody = $('#' + commentHtml.attr('id')).find('.fos_comment_comment_body').first();

      originalCommentBody.html(commentHtml.find('.fos_comment_comment_body').first().html());
    },
